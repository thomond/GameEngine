...
bool TMXConfigManager::load(std::string filename)
{
...
std::vector<Layer>* TMXConfigManager::getTileLayers()
{
    ...
    for(auto& layer : TMXTilelayer) {
        ...
        // Aquire properties and check for presence of stage_name
        auto props = layer->getProperties();
        int id;
        std::string state;
        if(!props.empty()) {
            for(auto prop : props) {
                if(!prop.getName().compare("stage_name") ) {
                    state = prop.getStringValue();
                    mLayer.stageName = state;
                }
            }
        }
